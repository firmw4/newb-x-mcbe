...
vec3 drawStar(vec3 color, vec3 viewDir, float time, vec3 fogColor, float rainFactor) {
  ...
  return color + (vec3(s, s, s)*NLC_TWINKLING_STAR) *mask;
}
...
vec3 drawTheEndStar(vec3 color, vec3 viewDir, float time) {
  ...
  float s = pow(clamp(randStar(viewDir*200.0), 0.0, 1.0), 8.0)*100.0;
  float f = 0.5 + 0.5 * sin(randStar(starDir*150.0 + vec3(time, time, time))*6.28 + time*0.5); 
  s *= mix(0.4, 1.4, randStar(starDir*100.0+vec3(time, time, time)))*f;

  return color + vec3(s, s, s) * NLC_END_TWINKLING_STAR;
}
